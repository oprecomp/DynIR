...
int refine_s_ad(OP A, OP b, OP x, LP LU, OP dp, int *ipiv, int N, int ir_info[2]) {	
	...
	//ADDED For TransFloat extenstion 
	float* sp_temp = (float *) malloc(N*sizeof(float));
		...
		{  //printf("\ninner solver from ad...\n");
			for (i = 0; i < 30; i++)
				{	
					...
					for(j=0;j<N;j++) {sp_temp[j] = (float)resid_temp[j];}; 
					sgetrs_(&trans, &N, &nrhs, LU, &N, ipiv, sp_temp, &N, info);
					del_z_norm = sdnormi(sp_temp, N);			
					BLAS_daxpby_s(N, 1, sp_temp, 1, 1, dp, 1);
